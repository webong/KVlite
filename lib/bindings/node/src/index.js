...
export function findLibrary(requestedPath) {
  const candidates = [requestedPath, process.env.KVLITE_LIBRARY_PATH];
  if (process.env.KVLITE_HOME) candidates.push(path.join(process.env.KVLITE_HOME, 'lib', libraryName()));
...
export class NativeDatabase {
  ...
  static open(databasePath, { libraryPath } = {}) {
    if (typeof databasePath !== 'string' || databasePath.length === 0) {
      throw new InvalidArgumentError('KVLite database path is required.');
    }
    const addon = loadAddon();
    try {
      return new NativeDatabase(addon, addon.open(databasePath, findLibrary(libraryPath)));
    } catch (error) {
...
export class HttpDatabase {
  constructor(baseUrl, { token, timeoutMs = 30_000, fetch: fetchImpl = globalThis.fetch } = {}) {
    let parsed;
    try {
