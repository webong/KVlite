...
func TestDriverPersistsCollectionsAndRecords(t *testing.T) {
	...
	db, err := kvlite.Open(path, kvlite.WithDriver("lmdb"))
	if err != nil {
		t.Fatal(err)
	}
	...
	if _, err := os.Stat(filepath.Join(path, "data.mdb")); err != nil {
		t.Fatal(err)
	}
	...
	if manifest.Backend != "lmdb" || manifest.Driver != "powerdns-lmdb-go" {
...
func TestModuleManifestMatchesLinkedMetadata(t *testing.T) {
	...
	if !reflect.DeepEqual(manifest, lmdb.Manifest()) {
		t.Fatalf("module mismatch: %#v", manifest)
	}
}

func TestOversizedStorageKeyHasActionableError(t *testing.T) {
	db, err := kvlite.Open(t.TempDir(), kvlite.WithDriver("lmdb"))
	if err != nil {
		t.Fatal(err)
	}
	t.Cleanup(func() { _ = db.Close() })
	err = db.Put(context.Background(), strings.Repeat("x", 1024), "value")
	if err == nil || !strings.Contains(err.Error(), "LMDB storage key") {
		t.Fatalf("Put() error = %v, want LMDB key-size guidance", err)
	}
}
