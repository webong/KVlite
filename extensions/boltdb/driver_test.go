...
func TestDriverPersistsCollectionsAndRecords(t *testing.T) {
	...
	db, err := kvlite.Open(path, kvlite.WithDriver("boltdb"))
	if err != nil {
		t.Fatal(err)
	}
	...
	if _, err := os.Stat(filepath.Join(path, boltdb.DatabaseFilename)); err != nil {
		t.Fatal(err)
	}
	...
	if manifest.Backend != "boltdb" || manifest.Driver != "bbolt" {
...
func TestModuleManifestMatchesLinkedMetadata(t *testing.T) {
	...
	if !reflect.DeepEqual(manifest, boltdb.Manifest()) {
