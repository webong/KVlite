...
import (
	...
	"github.com/webong/kvlite/extensions/rocksdb"
)
...
func TestModuleManifestMatchesLinkedMetadata(t *testing.T) {
	...
	if !reflect.DeepEqual(manifest, rocksdb.Manifest()) {
		t.Fatalf("source manifest = %#v, linked manifest = %#v", manifest, rocksdb.Manifest())
	}
	linked, err := kvlite.LinkedModule("rocksdb")
	if err != nil {
		t.Fatal(err)
	}
